...
class ShopController extends Controller
{
    ...
    public function show(Request $request, Product $product)
    {
        ...
        // Pastikan produk yang diakses adalah milik tenant ini (keamanan tambahan)
        if ($product->user_id !== $tenant->user_id) {
            abort(404);
        }

        $product->load('subCategory', 'variants', 'gallery', 'tags');
        $relatedProducts = Product::where('user_id', $tenant->user_id)
