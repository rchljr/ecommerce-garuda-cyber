            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($pendingPartners as $partner)
                    <tr class="text-gray-700 text-center">
                        <td class="px-6 py-4">
                            <div class="font-bold text-black">{{ optional($partner->shop)->shop_name ?? 'N/A' }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            @php
                                $categorySlug = optional($partner->shop)->product_categories;
                            @endphp
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-1 text-left">
                                @if(optional($partner->shop)->shop_photo) <a
                                    href="{{ Storage::url($partner->shop->shop_photo) }}" target="_blank"
                                class="text-sm text-blue-600 underline hover:text-blue-800">Foto Usaha</a> @endif
                                @if(optional($partner->shop)->ktp) <a href="{{ Storage::url($partner->shop->ktp) }}"
                                target="_blank" class="text-sm text-blue-600 underline hover:text-blue-800">KTP</a> @endif
                                @if(optional($partner->shop)->sku) <a href="{{ Storage::url($partner->shop->sku) }}"
                                target="_blank" class="text-sm text-blue-600 underline hover:text-blue-800">SKU</a> @endif

                                @if(optional($partner->shop)->npwp) <a href="{{ Storage::url($partner->shop->npwp) }}"
                                target="_blank" class="text-sm text-blue-600 underline hover:text-blue-800">NPWP</a> @endif
                                @if(optional($partner->shop)->nib) <a href="{{ Storage::url($partner->shop->nib) }}"
                                target="_blank" class="text-sm text-blue-600 underline hover:text-blue-800">NIB</a> @endif
                                @if(optional($partner->shop)->iumk) <a href="{{ Storage::url($partner->shop->iumk) }}"
                                target="_blank" class="text-sm text-blue-600 underline hover:text-blue-800">IUMK</a> @endif
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 text-sm font-semibold rounded-full bg-yellow-100 text-yellow-700">Menunggu
                                Verifikasi</span>
                        </td>
