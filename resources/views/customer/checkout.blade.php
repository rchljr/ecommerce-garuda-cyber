        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function handlePaymentResponse(data) {
                ...
                if (data.va_numbers && data.va_numbers.length > 0) {
                    ...
                    html = `<div class="p-6 border-l-4 border-blue-500 bg-blue-50 rounded-r-lg"><h4 class="font-bold text-lg text-blue-800">Instruksi Pembayaran ${va.bank.toUpperCase()} VA</h4><p class="mt-2 text-sm">Silakan selesaikan pembayaran Anda ke nomor Virtual Account berikut:</p><div class="my-4 p-3 bg-white border rounded-lg text-center flex items-center justify-between"><p id="va-number" class="text-xl md:text-2xl font-bold tracking-wider break-all">${va.va_number}</p><button class="ml-4 text-sm text-blue-600 hover:underline">Salin</button></div><p class="text-sm">Total Tagihan: <strong class="font-bold">${formatRupiah(data.gross_amount)}</strong></p><p class="text-xs mt-2">Batas waktu pembayaran: ${new Date(data.expiry_time).toLocaleString('id-ID')}</p></div>`;
