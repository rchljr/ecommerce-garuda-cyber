    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <div class="space-y-2 text-gray-600">
                    ...
                    <p><strong>Metode Pengiriman:</strong> <span
                            id="current-delivery-method-display">{{ ucfirst(str_replace('_', ' ', $order->delivery_method ?? 'Tidak Diketahui')) }}</span>
                    </p>
                    ...
                    <p><strong>Layanan Pengiriman:</strong> <span
                            id="current-delivery-service-display">{{ $order->shipping->delivery_service ?? 'Belum tersedia' }}</span>
                    </p>
                    <p><strong>Nomor Resi:</strong> <span
                            id="current-tracking-number-display">{{ $order->shipping->receipt_number ?? 'Belum tersedia' }}</span>
                    </p>
                    <p><strong>Catatan:</strong> {{ $order->notes ?? '-' }}</p>
                    @if ($order->voucher)
                        <p><strong>Voucher Digunakan:</strong> {{ $order->voucher->voucher_code }} (Diskon:
                            {{ $order->voucher->discount }}%)</p>
                    @endif
                    @if ($order->payment)
                        <p><strong>Status Pembayaran:</strong>
                            {{ ucfirst(str_replace('_', ' ', $order->payment->midtrans_transaction_status ?? 'N/A')) }} (Tipe:
                            {{ $order->payment->midtrans_payment_type ?? 'N/A' }})</p>
                    @endif
                </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($order->items as $item)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ format_rupiah($item->price) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ format_rupiah($item->price * $item->quantity) }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <div class="space-y-2 text-gray-600">
                    <p class="flex justify-between"><span>Subtotal:</span>
                        <span>{{ format_rupiah($order->subtotal) }}</span></p>
                    <p class="flex justify-between"><span>Biaya Pengiriman:</span>
                        <span>{{ format_rupiah($order->shipping_cost) }}</span></p>
                    <p class="flex justify-between"><span>Diskon:</span> <span>-
                            {{ format_rupiah($order->discount_amount) }}</span></p>
                    <p class="flex justify-between text-lg font-bold"><span>Total Pembayaran:</span>
                        <span>{{ format_rupiah($order->total_price) }}</span></p>
                </div>
            </div>

            {{-- [MODIFIKASI] Aksi Pesanan --}}
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            @if($order->status == 'processing')
                @if($order->delivery_method == 'pickup')
                    ...
                    <form action="{{ route('mitra.orders.updateStatus', $order->id) }}" method="POST" onsubmit="return confirm('Anda yakin pesanan ini sudah siap diambil?');">
                        @csrf
                        @method('PUT') {{-- PERBAIKAN: Diubah ke PUT --}}
                ...
                @elseif($order->delivery_method == 'ship')
                    {{-- Alur untuk metode SHIP/DELIVERY --}}
                    ...
                    <form action="{{ route('mitra.orders.updateStatus', $order->id) }}" method="POST">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" id="delivery-service-input" name="delivery_service" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('delivery_service', $order->shipping->delivery_service ?? '') }}" placeholder="Contoh: JNE REG, SiCepat BEST" required>
                            </div>
                            <div>
                                <label for="receipt-number-input" class="block text-sm font-medium text-gray-700">Nomor Resi Pengiriman</label>
                                <input type="text" id="receipt-number-input" name="receipt_number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('receipt_number', $order->shipping->receipt_number ?? '') }}" placeholder="Masukkan nomor resi" required>
                            </div>
                            <button type="submit" class="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 transition">
