...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white shadow-md rounded-lg p-6 mb-6">
        ...
        <form action="{{ route('mitra.transactions.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end" id="filter-form">
            <div>
                <label for="date_range" class="block text-sm font-medium text-gray-700">Rentang Tanggal:</label>
                <select id="date_range" name="date_range" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
            ...
            <div class="{{ $dateRange === 'custom' ? 'hidden' : '' }}"></div> {{-- Placeholder if custom inputs are shown --}}
            <div class="{{ $dateRange === 'custom' ? 'hidden' : '' }}"></div> {{-- Placeholder if custom inputs are shown --}}
            ...
            <div>
                ...
                <input type="text" id="customer_name" name="customer_name" value="{{ request('customer_name') }}"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200" placeholder="Cari nama pelanggan">
            </div>
            <div>
                ...
                <select id="delivery_method" name="delivery_method"
                    ...
                    <option value="delivery" {{ request('delivery_method') == 'delivery' ? 'selected' : '' }}>Diantar</option>
                    <option value="pickup" {{ request('delivery_method') == 'pickup' ? 'selected' : '' }}>Dijemput di Toko</option>
                </select>
            </div>
            <div>
                <label for="delivery_service" class="block text-sm font-medium text-gray-700">Layanan Pengiriman:</label>
                <select id="delivery_service" name="delivery_service"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                    <option value="">Semua</option>
                    @foreach($deliveryServices as $service)
                        <option value="{{ $service }}" {{ request('delivery_service') == $service ? 'selected' : '' }}>{{ $service }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-span-1 md:col-span-2 lg:col-span-4 flex gap-4">
                ...
                <a href="{{ route('mitra.transactions.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                    <i class="fas fa-sync-alt mr-2"></i> Reset Filter
                </a>
                
                {{-- BARU: Tombol Export untuk Laporan Produk --}}
                <a href="{{ route('mitra.transactions.export.excel', request()->query()) }}" class="inline-flex items-center px-4 py-2 border border-green-300 text-sm font-medium rounded-md shadow-sm text-green-700 bg-green-50 hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                    <i class="fas fa-file-excel mr-2"></i> Export Laporan Produk
                </a>

                {{-- OPSI: Jika Anda ingin tombol export langganan di halaman ini juga --}}
                {{-- <a href="{{ route('mitra.pendapatan.export.excel', request()->query()) }}" class="inline-flex items-center px-4 py-2 border border-purple-300 text-sm font-medium rounded-md shadow-sm text-purple-700 bg-purple-50 hover:bg-purple-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Modal</th>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Metode Pengiriman</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Info Resi</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($completedOrders as $order)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-semibold">{{ format_rupiah($order->total_price) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-red-600">{{ format_rupiah($order->calculated_cost) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-blue-600 font-semibold">{{ format_rupiah($order->calculated_profit) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ucfirst($order->payment->midtrans_transaction_status ?? 'N/A') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ucfirst(str_replace('_', ' ', $order->delivery_method ?? 'Tidak Diketahui')) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    @if ($order->delivery_method == 'delivery' && $order->shipping && $order->shipping->receipt_number)
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">Tidak ada transaksi yang selesai.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
