...
@section('content')
    <div class="bg-white shadow-xl rounded-xl p-6 sm:p-8 md:p-10 max-w-screen-xl w-full mx-auto">
        ...
        <div class="flex justify-between items-center mb-6 w-full">
            ...
            <!-- Tombol Tambah Hero Baru ke kanan -->
            <a href="{{ route('mitra.heroes.create') }}"
                        ...
                        @foreach ($heroes as $hero)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $hero->title }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $hero->subtitle ?? '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $hero->order }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $hero->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-3">
                                        {{-- Ikon Edit (SVG) --}}
                                        <a href="{{ route('mitra.heroes.edit', $hero->id) }}" title="Edit Hero"
                                        ...
                                        {{-- Ikon Hapus (SVG) --}}
                                        <form action="{{ route('mitra.heroes.destroy', $hero->id) }}" method="POST"
                                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus item hero ini? Ini tidak dapat dibatalkan.');"
                                            class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" title="Hapus Hero"
