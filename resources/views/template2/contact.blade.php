...
<section class="ftco-section contact-section bg-light">
    <div class="container">
        ...
        <div class="row d-flex mb-5 contact-info">
            ...
            <div class="col-md-3 d-flex">
                <div class="info bg-white p-4 w-100">
                    <p><span>Telepon:</span> <a href="tel://{{ optional($contact)->phone ?? '' }}">{{ optional($contact)->phone ?? 'N/A' }}</a></p>
                </div>
            </div>
            <div class="col-md-3 d-flex">
                <div class="info bg-white p-4 w-100">
                    <p><span>Email:</span> <a href="mailto:{{ optional($contact)->email ?? '' }}">{{ optional($contact)->email ?? 'N/A' }}</a></p>
                </div>
            </div>
            <div class="col-md-3 d-flex">
                <div class="info bg-white p-4 w-100">
                    <p><span>Website</span> <a href="{{ optional($contact)->website ?? '#' }}" target="_blank">{{ optional($contact)->website ? basename(optional($contact)->website) : 'Belum diatur' }}</a></p>
                </div>
            </div>
        </div>
        <div class="row block-9">
            <div class="col-md-6 order-md-last d-flex">
                ...
                <form action="{{ !$isPreview ? route('tenant.contact', ['subdomain' => $currentSubdomain]) : '#' }}" method="POST" class="bg-white p-5 contact-form">
                    @csrf
                    ...
                    <div class="form-group">
                        <textarea name="message" cols="30" rows="7" class="form-control" placeholder="Isi Pesan" required>{{ old('message') }}</textarea>
                        @error('message') <span class="text-danger small">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                        <button type="submit" class="btn btn-primary py-3 px-5" {{ $isPreview ? 'disabled' : '' }}>Kirim Pesan</button>
                    </div>
                </form>
            </div>
