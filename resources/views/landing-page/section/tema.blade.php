...
<section id="tema" class="py-16 md:py-24">
    <div class="container mx-auto px-6">
        <div class="text-center mb-12">
            ...
            <p class="mt-4 text-lg text-gray-600">Pilih dari beragam desain profesional yang siap pakai untuk bisnis Anda.</p>
        </div>
        <div class="border-2 border-red-300 rounded-3xl p-4 md:p-8">
            <div class="relative">
                <div id="tema-carousel-container" class="overflow-hidden">
                    {{-- PERBAIKAN: Menggunakan class 'no-scrollbar' untuk menyembunyikan scrollbar bawaan --}}
                    <div id="tema-carousel" class="flex space-x-6 pb-4 overflow-x-auto no-scrollbar scroll-smooth">
                        
                        @forelse($templates as $template)
                            {{-- PERBAIKAN: Kelas lebar disederhanakan untuk layout 3 kolom di desktop --}}
                            <div class="tema-card flex-shrink-0 w-4/5 sm:w-1/2 md:w-[calc(33.333%-1rem)] bg-white rounded-2xl shadow-md p-4 text-center flex flex-col">
                                <a href="{{ route('template.preview', $template) }}" target="_blank" class="block">
                                    <img src="{{ asset('storage/' . $template->image_preview) }}" 
                                         ...
                                         alt="{{ $template->name }}" class="w-full h-40 object-cover rounded-lg mb-4 hover:opacity-80 transition-opacity">
                                </a>
                                <div class="flex flex-col flex-grow text-left">
                                    <h4 class="font-semibold text-gray-800">{{ $template->name }}</h4>
                                    <p class="text-gray-600 mt-2 text-sm flex-grow">{{ Str::limit($template->description, 85) }}</p>
                                </div>
                            </div>
                        @empty
